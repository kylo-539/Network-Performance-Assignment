...
//------------------------------------------------------
TypeId 
WiFiTimestampTag::GetTypeId (void)
{
  static TypeId tid = TypeId ("WiFiTimestampTag")
    .SetParent<Tag> ()
    .AddConstructor<WiFiTimestampTag> ()
    .AddAttribute ("Timestamp",
                   "Some momentous point in time!",
                   EmptyAttributeValue (),
                   MakeTimeAccessor (&WiFiTimestampTag::GetTimestamp),
                   MakeTimeChecker ())
  ;
  return tid;
}
TypeId 
WiFiTimestampTag::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t 
WiFiTimestampTag::GetSerializedSize (void) const
{
  return 8;
}
void 
WiFiTimestampTag::Serialize (TagBuffer i) const
{
  int64_t t = m_timestamp.GetNanoSeconds ();
  i.Write ((const uint8_t *)&t, 8);
}
void 
WiFiTimestampTag::Deserialize (TagBuffer i)
{
  int64_t t;
  i.Read ((uint8_t *)&t, 8);
...
void
WiFiTimestampTag::SetTimestamp (Time time)
{
  m_timestamp = time;
}
Time
WiFiTimestampTag::GetTimestamp (void) const
{
  return m_timestamp;
}

void 
WiFiTimestampTag::Print (std::ostream &os) const
{
  os << "t=" << m_timestamp;
}
